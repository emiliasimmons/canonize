...
**A finding from an analysis you ran.** Write it into `docs/evidence/findings/` with enough detail to understand and re-run it: the question, the method as a pointer to the re-runnable artifact (never paste the script), the inputs that mattered, and the result with its diagnostic. When the project has registered workspaces (check `docs/schema.md`), write to this repo's workspace subdirectory: `docs/evidence/findings/<workspace>/`. The workspace name for the current repo is recorded in the `## docs` steering block of `CLAUDE.md`. If the project has no registered workspaces, write to `docs/evidence/findings/` directly. The filename should mirror the analysis artifact it came from. Findings are immutable once written: append, re-run, or supersede, never quietly rewrite. A validation result lands here like any other finding.
